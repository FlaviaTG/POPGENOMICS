...
mydata2=read.table("bayescan-output_fst.txt",colClasses="numeric", header=TRUE)
plot_bayescan(mydata2,FDR=0.05)
results <-plot_bayescan("bayescan-output_fst.txt",0, FDR=0.05)
results$outliers
results$nb_outliers
###Fst distribution######
mydata=read.table("bayescan-output.baye.sel",colClasses="numeric", header=TRUE)
...
boa.hpd(mydata[["Fst5"]],0.05)
boa.hpd(mydata[["Fst6"]],0.05)
boa.hpd(mydata[["Fst7"]],0.05)
boa.hpd(mydata[["Fst8"]],0.05)
boa.hpd(mydata[["Fst9"]],0.05)
###########
plot(density(mydata2[["alpha"]]),xlab="alpha",main=paste("alpha","posterior distribution"))
