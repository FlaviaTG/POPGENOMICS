...
##Then you will need to produce a database of all intervals for all sample in a array again but this time you just need ONE JOB ARRAY WITH ALL YOUR SAMPLES MENTIONED IN THERE. First it will create a database
...
###then joint genotyping and produce a multi-sample variant call-set for the databases per interval. See the job script: Job-GATK-GVCF-Genotype_scaffold. In general it will take all samples together per interval=chromosomes/scaffolds/contigs
